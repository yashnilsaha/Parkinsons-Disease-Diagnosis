...
ax.plot(horizontal_displacemt_array,label='Horizontal motion for left ankle for record id 26')
ax.legend(loc='best')

ax2 = fig.add_subplot(122)
ax2.plot(vertical_displacement_array,label='vertical motion for left ankle for record id 26')
ax2.legend(loc='best')
...
processed_df = pd.DataFrame(columns=["combine_record_id", "position_name", "sub_score",
                                     "speed_max", "speed_median", "speed_mean", "speed_std_div", "speed_iqr_range",
...
for sub_score, sub_score_group in sub_score_gr:
    ...
    X = sub_score_group.drop(['position_name', 'combine_record_id', 'sub_score', 'UDysRS_rating'], axis=1)
