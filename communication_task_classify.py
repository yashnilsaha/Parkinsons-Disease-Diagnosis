...
def get_spectral_feature(signals, sample_frequency=10, is_plot=False):
    ...
    def bandpower(psd, freqs, min_freqs, max_freqs, is_plot=False):
        ...
        if is_plot:
            ...
            plt.title('Power Spectral Density for head joint')
            sns.despine()
...
def record_convertion(position_array, position_name, index, record_id="1-1"):
    ...
    #if index == 4:
    #    spectral_feature_displacement = get_spectral_feature(displacement_array, is_plot=True)
    #else:
...
ax.plot(horizontal_displacement_array,label='Horizontal motion')
ax.legend(loc='best')
...
for record_id, values in comm_dict.items():
    ...
    for position_name, position_array in positions.items():
        row = record_convertion(position_array, position_name, index, record_id)
...
def plot_roc_curve(fpr, tpr, joint):
  ...
  plt.title('ROC curve for joint '+joint+' in communication task')
  plt.show()
...
processed_df.drop(["combine_record_id", "record_id", "term", "position_name"
                  ], axis = 1, inplace = True)
...
for sub_score, sub_score_group in sub_score_gr:
    ...
    X = sub_score_group[["sub_score","convexhull","speed_max", "speed_median", "speed_mean", "speed_std_div", "speed_iqr_range",
                                     "acceleration_max", "acceleration_median", "acceleration_mean",
                                     "acceleration_std_div", "acceleration_iqr_range",
                                     "jerk_max", "jerk_median", "jerk_mean", "jerk_std_div", "jerk_iqr_range",
                                     "displacement_peak_magnitude", "displacement_entropy", "displacement_half_point",
    ...
    #x_array_train = sklearn.preprocessing.normalize(np.asarray(X_train).astype(np.float32), axis=0)
    x_array_train = np.asarray(X_train).astype(np.float32)
    y_array_train = np.asarray(y_train).astype(np.float32)
    #x_array_test = sklearn.preprocessing.normalize(np.asarray(X_test).astype(np.float32), axis=0)
    ...
    model_4.compile(loss=tf.keras.losses.binary_crossentropy,
                    ...
                    metrics=['accuracy', f1_m, tf.keras.metrics.Precision(name='precision'),tf.keras.metrics.Recall(name='recall')])

    history = model_4.fit(x_array_train, y_array_train, epochs = 20,  batch_size=128,verbose = 0,validation_split=0.2)
    ...
    plt.ylabel('Actuals')
    plt.title('Communication: Confusion Matrix for '+joint, fontsize=18)
    #plt.show()
    ...
    plt.xlabel('Epochs')
    plt.ylabel('F1-score')
    plt.title('Model training for the joint '+joint+' in communication task')
    plt.legend()
    fig = plt.figure()
    fig.savefig('acc.png')
