...
print(comm_dict['217'].keys())
print(sorted(comm_dict['217'].keys()))

plt.plot(np.array(comm_dict['217']['position']['Lank_act'])[:,0]) # visualizing the horizontal movement
plt.suptitle('Leg Agility task showing the horizontal movement for record id 217')
plt.show()


part = "Lank_act"
horizontal_displacemt_array = np.array(comm_dict['217']['position'][part])[:,0]
vertical_displacement_array = np.array(comm_dict['217']['position'][part])[:,1]
...
plt.suptitle('Joint ankle motion')
plt.show()
...
for record_id, values in comm_dict.items():
    #position values are being used below
...
sub_score_dict = {"Right": ["Rhip_act", "Rkne_act", "Rank_act"],
                  "Left": ["Lhip_act","Lkne_act","Lank_act"]
                 }
...
def find_rating(record_id, sub_group):
    ...
    try:
        rating = ratings['3.8'][str(record_id)][order[sub_group]]
    except:
        rating = 1
    return rating
...
for sub_score, sub_score_group in sub_score_gr:
    ...
    #  Model uses six layers.
    # The first layer consists of 256 hidden units/ neurons with the ReLU activation function.
    # ReLU stands for Rectified Linear Units.
    # The second layer is a Dropout with a probability p = 0.2
    # The third layer consists of 480 hidden units with the ReLU activation function.
    #  The fourth layer is a Dropout with a probability p = 0.2
    # The fifth layer consists of 256 hidden units with the ReLU activation function.
    ...
    # loss function
    # Mean Squared Logarithmic Loss as loss function and metric of rmse, and Adam loss function optimizer
