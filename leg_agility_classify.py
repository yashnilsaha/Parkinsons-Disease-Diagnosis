...
part = "Lank_act"
horizontal_displacement_array = np.array(comm_dict['217']['position'][part])[:,0]
vertical_displacement_array = np.array(comm_dict['217']['position'][part])[:,1]
fig = plt.figure(figsize=(15, 4))
ax = fig.add_subplot(121)
ax.plot(horizontal_displacement_array,label='horizontal left ankle motion')
ax.legend(loc='best')
...
def find_rating(record_id, sub_group):
    ...
    try:
        #using UPDRS Part 3.8
        rating = ratings['3.8'][str(record_id)][order[sub_group]]
        #print("rating :", rating)
    except:
        print("record id with exception :", record_id)
        rating = 1
    return rating
    ...
    for index, dict_ in group.iterrows():
        ...
        for sub_score, values in sub_score_dict.items():
            if position_name in values:
                ...
                #if (UPDRS_ratingVal) > 2 and (UPDRS_ratingVal) < 3:
                if (UPDRS_ratingVal) >= 1.0:
...
print("****Classification of Leg Agility using Neural Network****")
...
for sub_score, sub_score_group in sub_score_gr:
    ...
    history = model_4.fit(x_array_train, y_array_train, epochs=50, batch_size=128, verbose=0, validation_split=0.2)
    ...
    plt.ylabel('Actuals')
    plt.title('Leg Agility: Confusion Matrix for '+joint, fontsize=18)
    plt.show()
    ...
    plt.xlabel('Epochs')
    plt.ylabel('F1-score')
    plt.title('Model training for the joint ' + joint + ' in leg agility task')
    plt.legend()
    fig = plt.figure()
    fig.savefig('acc.png')
